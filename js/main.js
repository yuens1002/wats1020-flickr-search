...
$(document).on('ready', function() {
	...
	var searchImages = function(tags) {
			...
			$.getJSON(flickrAPI, {
				...
				$('#images').empty();
				$('<p class="lead">Showing Images of <strong id="keyword"></strong>').appendTo('#images');
				$('#keyword').text(tags);
				...
				$.each(data.items, function(i, item) {				
					...
					$('<p>').addClass('text-capitalize').html('<strong>' + item.title + '</strong>').appendTo(inner);
					$('<p>').html(formatDate(item.date_taken) + '<br>' + formatAuthor(item.author)).appendTo(inner);
